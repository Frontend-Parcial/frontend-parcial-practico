                        ...
                        {solicitudes.map((solicitud) => (
                          <tr key={solicitud._id} className='hover:bg-gray-50'>
                            ...
                            <td className='px-6 py-4 whitespace-nowrap'>
                              ...
                              <div className='text-sm text-gray-500'>{solicitud.convenio.pais} ({solicitud.convenio.tipo})</div>
                            </td>
                            ...
                            <td className='px-6 py-4 whitespace-nowrap'>
                              <span className={`px-2 py-1 text-xs font-semibold rounded-full ${getEstadoColor(solicitud.estado_solicitud)}`}>
                    ...
                    <div className='hidden sm:flex-1 sm:flex sm:items-center sm:justify-between'>
                      <div>
                        <p className='text-sm text-gray-700'>
                          Mostrando <span className='font-medium'>{(pagination.page - 1) * pagination.perPage + 1}</span> a{' '}
                          <span className='font-medium'>{Math.min(pagination.page * pagination.perPage, pagination.total)}</span> de{' '}
                          ...
                          {Array.from({ length: Math.min(5, pagination.pages) }, (_, i) => {
                            ...
                            return (
                              <button
                                ...
                                className={`relative inline-flex items-center px-4 py-2 border text-sm font-medium ${pagination.page === pageNum ? 'z-10 bg-primario border-primario text-complementario' : 'bg-white border-gray-300 text-gray-500 hover:bg-gray-50'}`}
                              >
                                {pageNum}
                              </button>
                            );
